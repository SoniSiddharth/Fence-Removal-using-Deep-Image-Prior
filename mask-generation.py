...
path = "./data/inpainting/"
input_image = 'man-fence'

img = cv2.imread(path+input_image+'.jpg')
try:
    if img==None:
        img = cv2.imread(path+input_image+'.png')
except:
    pass
...
img_erosion = cv2.erode(out_binary, kernel, iterations=2)
...
cv2.imwrite(path+"TEC-"+ input_image +"-mask.png", image)
cv2.imshow("mask-generated", image)
